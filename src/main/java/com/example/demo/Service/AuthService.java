...
public class AuthService {
    ...
    public String generateToken(User user) {
        ...
        if (existingToken != null && now.isBefore(existingToken.getExpiresat())) {
            token = existingToken.getJwttoken();
        } else {
    ...
    public boolean validateToken(String token) {
        try {
            ...
            Optional<Token> jwtToken = jwtTokenRepository.findByJwttoken(token);
    ...
    public String extractUsername(String token) {
        return Jwts.parser().setSigningKey(SIGNING_KEY).build().parseClaimsJws(token).getBody().getSubject();
    }
}